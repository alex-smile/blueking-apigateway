...
class RequestThirdPartyException(BaseException):
    ...
    def __str__(self):
        return (
            "Component request third-party system [%s] interface [%s] error: %s, "
...
class RequestSSLException(BaseException):
    ...
    def get_message(self):
        ...
        return (
            "Component request third-party system [%s] interface [%s] SSL error: "
...
class HostNotFoundException(BaseException):
    """
    组件的 host 未指定
    """
...
class ErrorCodes(object):
    ...
    error_codes = (
        ...
        ErrorCode(
            "BUFFET_CANNOT_FORMAT_PATH", 1306407, "The component's destination request path cannot be formatted"
        ),
        ErrorCode("APIGW_ERROR", 1306408, "APIGateway JWT verification failed"),
        ...
        ErrorCode("HOST_NOT_FOUND", 1306206, "Error, the component does not configure the api host"),
        ...
        ErrorCode("REQUEST_DEST_METHOD_ERROR", 1306209, "The system interface does not support the request method"),
    )
