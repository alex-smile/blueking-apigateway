...
from past.builtins import basestring  # noqa: E402
from requests.exceptions import ReadTimeout, SSLError  # noqa: E402

from common.base_utils import FancyDict, datetime_format, urljoin  # noqa: E402
from common.bkerrors import bk_error_codes  # noqa: E402
from common.errors import HostNotFoundException, RequestSSLException, RequestThirdPartyException  # noqa: E402
    ...
    def _request(
        ...
        try:
            ...
            if not allow_non_200 and resp_status_code != STATUS_CODE_OK:
                raise Exception(
                    f"Status Code: {resp_status_code}, "
                    "Error Message: third-party system interface response status code is not 200"
                )
        ...
        if isinstance(host, SmartHost):
            ...
            if use_test_env and not host.has_test_host():
                raise HostNotFoundException(
                    "Error, the component does not support access third-party test environment"
                )

            host = host.get_value(use_test_env=use_test_env)

        if not host:
            raise HostNotFoundException("Error, the component does not configure the api host")
...
class HttpClient(BasicHttpClient):
    ...
    def prepare_bk_header(self, headers={}, with_jwt_header=False):
        ...
        if self.component.request:
            ...
            if with_jwt_header:
                # 如果 X-Bkapi-Jwt 使用来源于 API Gateway 的数据，需注意：
                # - 前端人员选择器发送的请求，应用的 verified=False，如需替换以下逻辑，ESB 需生成新的 Jwt
    ...
    def request(
        ...
        if r.request_exception:
            if isinstance(r.request_exception, SSLError):
                ...
                logger.error(
                    "%s request third party SSLError, system_name: %s, ssl_root_dir: %s",
